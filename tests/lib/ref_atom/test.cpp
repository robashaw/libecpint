...
int main() {
  ...
  ECP UA(O);
  UA.addPrimitive(2, 0, 1.5, 2.0);
  UA.addPrimitive(3, 0, 0.7, 0.4);
  UA.addPrimitive(2, 1, 1.2, -1.0);
  UA.addPrimitive(2, 2, 0.9, 0.5);
  UA.addPrimitive(4, 2, 0.6, 0.25, true);
  ...
  ECP UB(O);
  UB.addPrimitive(2, 0, 1000.0, 1.0);
  UB.addPrimitive(2, 0, 100000.0, 0.5, true);
  ...
  std::vector<Pair> pairs = {
      {&UA, &sA, &sA}, {&UA, &pA, &sA}, {&UA, &pA, &pA}, {&UA, &dA, &sA}, {&UA, &dA, &pA},
      {&UA, &dA, &dA}, {&UB, &sB, &sB}, {&UB, &pB, &sB}, {&UB, &pB, &pB}, {&UB, &dB, &sB},
      {&UB, &dB, &pB}, {&UB, &dB, &dB}, {&UC, &sC, &sC}, {&UC, &pC, &sC}, {&UC, &pC, &pC},
  };
  ...
  if (!in.is_open()) {
    std::cerr << "cannot open ref_atom.output" << std::endl;
    return 1;
  }
  std::vector<double> ref;
  std::string line;
  while (std::getline(in, line))
    if (!line.empty()) ref.push_back(std::stod(line));
  ...
  for (size_t i = 0; i < ref.size(); i++) {
    if (std::abs(ref[i]) > 1e-30) {
      ...
      if (rel > reltol) {
        nfail++;
        std::cout << "FAIL line " << i << " ref=" << std::setprecision(8) << ref[i]
                  << " got=" << got[i] << " rel=" << rel << "\n";
      }
    } else {
      worst_abs = std::max(worst_abs, std::abs(got[i]));
      if (std::abs(got[i]) > abstol) {
        nfail++;
        std::cout << "FAIL line " << i << " expected 0 got=" << got[i] << "\n";
      }
    }
  }
  std::cout << "ref_atom: " << (ref.size() - nfail) << "/" << ref.size()
            << " ok; worst rel(nonzero)=" << worst_rel << " worst abs(zero)=" << worst_abs
            << std::endl;
  return nfail == 0 ? 0 : 1;
}
