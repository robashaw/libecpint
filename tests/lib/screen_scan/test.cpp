...
int main() {
  ...
  // closed forms
  auto s_exact = [](int n, double c) {
    return n == 1 ? 2.0 * M_PI / c : std::pow(M_PI, 1.5) / std::pow(c, 1.5);
  };
  auto p_exact = [](int n, double c) {
    return n == 1 ? 2.0 * M_PI / (3.0 * c * c) : std::pow(M_PI, 1.5) / (2.0 * std::pow(c, 2.5));
  };
    ...
    for (double alpha : {1e-2, 1.0, 1e2, 1e4}) {
      for (double g : {1e0, 1e2, 1e4, 1e6, 1e7}) {
        ...
        // s | U | s
        {
          ...
          ECP U(O);
          U.addPrimitive(n, 0, g, 1.0, true);
          GaussianShell s(O, 0);
          s.addPrim(alpha, 1.0);
          ECPIntegral ecpint(0, 0, 0, thresh);
          TwoIndex<double> r;
          ecpint.compute_shell_pair(U, s, s, r);
          ...
          if (std::abs(ex) > floor) {
            ...
            if (rel > reltol) {
              nfail++;
              std::cout << "FAIL s n=" << n << " a=" << alpha << " g=" << g << " got=" << r(0, 0)
                        << " exact=" << ex << " rel=" << rel << "\n";
            }
          }
        }
        // p_x | U | p_x
        {
          ...
          ECP U(O);
          U.addPrimitive(n, 0, g, 1.0, true);
          GaussianShell px(O, 1);
          px.addPrim(alpha, 1.0);
          ECPIntegral ecpint(1, 0, 0, thresh);
          TwoIndex<double> r;
          ecpint.compute_shell_pair(U, px, px, r);
          double rel = std::abs(r(0, 0) - ex) / std::abs(ex);
          if (std::abs(ex) > floor) {
            worst = std::max(worst, rel);
            ntot++;
            if (rel > reltol) {
              nfail++;
              std::cout << "FAIL p n=" << n << " a=" << alpha << " g=" << g << " got=" << r(0, 0)
                        << " exact=" << ex << " rel=" << rel << "\n";
            }
          }
        }
