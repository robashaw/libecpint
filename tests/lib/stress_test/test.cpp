...
double ag_4_centers[12] = {0.0, 0.0, 4.5, 0.0, 0.0, -4.5, 0.0, 2.4, 0.0, 0.0, -2.4, 0.0};
...
std::vector<double> run_bench(int n_centers, double* centers, std::string& share_dir) {
  ...
  timings.push_back(std::chrono::duration<double, std::deci>(last_time - first_time).count() /
                    10.0);
  std::cout << std::setw(20) << "done. TIME TAKEN: " << std::setw(15) << timings[0] << " seconds"
            << std::endl;
  ...
  timings.push_back(std::chrono::duration<double, std::deci>(last_time - first_time).count() /
                    10.0);
  std::cout << std::setw(20) << "done. TIME TAKEN: " << std::setw(15) << timings[1] << " seconds"
            << std::endl;
  ...
  if (LIBECPINT_MAX_L > 3) {
    ...
    timings.push_back(std::chrono::duration<double, std::deci>(last_time - first_time).count() /
                      10.0);
    std::cout << std::setw(20) << "done. TIME TAKEN: " << std::setw(15) << timings[2] << " seconds"
              << std::endl;
    if (LIBECPINT_MAX_L > 4) {
      if (n_centers < 4) {
        ...
        timings.push_back(std::chrono::duration<double, std::deci>(last_time - first_time).count() /
                          10.0);
        std::cout << std::setw(20) << "done. TIME TAKEN: " << std::setw(15) << timings[3]
                  << " seconds" << std::endl;
      }
    } else {
...
int main(int argc, char* argv[]) {
  ...
  std::cout << std::setw(30) << "Scaling of integrals: N**" << std::setprecision(3)
            << ysum_int / nsum2 << std::endl;
  std::cout << std::setw(30) << "Scaling of 1st derivs: N**" << std::setprecision(3)
            << ysum_deriv / nsum2 << std::endl;
}
