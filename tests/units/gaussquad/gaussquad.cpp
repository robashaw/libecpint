...
static std::vector<double> tabulate(GCQuadrature* grid, double (*f)(double, const double*, int),
                                    const double* coef) {
...
TEST_P(QuadTest, IntegratePoly) {
  ...
  const auto integral_and_test = grid_->integrate(params.data(), 1e-6, 0, grid_->getN() - 1);
...
INSTANTIATE_TEST_SUITE_P(GaussQuad, QuadTest, Values(&CreateBigGrid, &CreateSmallGrid));
